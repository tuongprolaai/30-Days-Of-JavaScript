...
const rectangle:{[key: string]: number} = {
  width: 20,
  height: 10,
  area: 200,
  perimeter: 60
}
const users: {name: string; scores: number; skills: string[]; age: number}[] = [
{
  name:'Brook',
  scores:75,
  skills:['HTM', 'CSS', 'JS'],
...
// Exercises: Level 3
 const countrieS: {name: string; capital: string; languages: string[]; population: number; flag: string; currency: string}[] = [
  {
    name: 'Afghanistan',
    capital: 'Kabul',
...
function convertArrayToObject(students:(string|string[]|number[])[][]){
    return students.map(([name, skills,scores])=>({
        name: name as string,  skills: skills as string[], scores: scores as number[]
    }));
}
const students = [
