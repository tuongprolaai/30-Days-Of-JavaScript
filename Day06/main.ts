...
// 11. Use for loop to iterate from 0 to 100 and print the sum of all evens and the sum of all odds.
{
  ...
  for (let i: number = 0; i <= 100; i++) {
    if (i % 2 === 0) sumE += i;
    else if (i % 2 !== 0) sumO += i;
  }
  console.log(
    `The sum of all evensfrom 0 to 100 is ${sumE}. And the sum of all odds from 0 to 100 is ${sumO}`
  );
}
...
// 14. Develop a small script which generate array of 5 random numbers and the numbers must be unique
{
  ...
  for (let i: number = 1; i <= 5; i++) {
    ...
    if (!arrNumberRandom.includes(numRandom)) arrNumberRandom.push(numRandom);
  }
  console.log(arrNumberRandom);
}
...
let id: string = "";
while (id.length < 6) {
  const xRandom: number = Math.floor(Math.random() * (122 - 48 + 1) + 48);
  if (
    (xRandom >= 48 && xRandom <= 57) ||
...
{
  const countriesTemp: (string | number)[][] = [];
  for (let country of countries) {
    const countryTemp: (string | number)[] = [];
...
let maxLengthCountry: string = countries[0]!;
for (let country of countries) {
  if (maxLengthCountry.length < country.length) maxLengthCountry = country;
}
...
for (let country of countries) {
  if (country.length === 5) countriesTemp.push(country);
}
console.log(countriesTemp);

// 11. Find the longest word in the webTechs array
let maxLengthWeb: string = webTechs[0]!;
for (let web of webTechs) {
  if (maxLengthWeb.length < web.length) maxLengthWeb = web;
}
console.log(maxLengthWeb);

// 12. Use the webTechs array to create the following array of arrays:
{
  const webTemp: (string | number)[][] = [];
  for (let web of webTechs) {
    const wTemp: (string | number)[] = [];
    wTemp.push(web);
...
{
  const countriesCoppy: string[] = [...countries];
}

// 2. Arrays are mutable. Create a copy of array which does not modify the original. Sort the copied array and store in a variable sortedCountries
{
  const countriesCoppy: string[] = [...countries].sort();
}

// 3. Sort the webTechs array and mernStack array
{
  const webTechsCoppy: string[] = [...webTechs].sort();
  const mernStackCoppy: string[] = [...mernStack].sort();
}

// 4. Extract all the countries contain the word 'land' from the countries array and print it as array
{
  const countriesWithLand: string[] = [];
...
{
  const fourLetterCountries: string[] = [];
  for (const country of countries) {
    if (country.length === 4) {
...
const multiWordCountries: string[] = [];
for (const country of countries) {
  if (country.includes(" ")) {
