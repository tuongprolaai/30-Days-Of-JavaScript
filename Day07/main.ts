...
{
  const addNumbers = (valueA: number, valueB: number): number =>
    valueA + valueB;
}
...
{
  const perimeterOfRectangle = (length: number, width: number): number =>
    2 * (length + width);
}

// 6. A volume of a rectangular prism is calculated as follows: volume = length x width x height. Write a function which calculates volumeOfRectPrism.
{
  const volumeOfRectPrism = (l: number, w: number, h: number): number =>
    l * w * h;
}

// 7. Area of a circle is calculated as follows: area = π x r x r. Write a function which calculates areaOfCircle
{
  const areaOfCircle = (r: number): number => {
...
{
  const circumOfCircle = (r: number): number => 2 * Math.PI * r;
}

// 9. Density of a substance is calculated as follows:density= mass/volume. Write a function which calculates density.
{
  const calculateDensity = (mass: number, volume: number): number =>
    mass / volume;
}
...
const checkSeason = (month: string): string => {
...
{
  const findMax = (a: number, b: number, c: number): number => {
...
{
  function swapValues(x: any, y: any): void {
    [x, y] = [y, x];
    console.log(`x = ${x}, y = ${y}`);
  }
}
...
{
  function removeItem(index: number, arr: any[]): any[] {
    const newArray = [...arr];
...
function sumOfOdds(n: number): number {
  let sum = 0;
  for (let i = 1; i <= n; i++) {
    if (i % 2 !== 0) {
      sum += i;
...
function evensAndOdds(n: number): void {
  let evens = 0;
  let odds = 0;
  for (let i = 0; i <= n; i++) {
    if (i % 2 === 0) {
...
function rgbColorGenerator(): string {
...
{
  function arrayOfHexaColors(count: number): string[] {
...
{
  function arrayOfRgbColors(count: number): string[] {
    const colors = [];
...
{
  function convertHexaToRgb(hexa: string): string {
...
{
  function convertRgbToHexa(r: number, g: number, b: number): string {
...
{
  function generateColors(type: string, count: number): string | string[] {
    const chars = "0123456789abcdef";
    const colors: string[] = [];
